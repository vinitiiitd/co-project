...
string reg[32] = {"x0", "x1", "x2", "x3", "x4", "x5", "x6", "x7", "x8", "x9", "x10", "x11", "x12", "x13", "x14", "x15",
                  "x16", "x17", "x18", "x19", "x20", "x21", "x22", "x23", "x24", "x25", "x26", "x27", "x28", "x29", "x30", "x31"};
int pc = 0;

bool isPresent(string arr[], int size, string target) {
...
string sext(int number, int target_length) {
...
// cout<< b_to_d(stoi(value[reg[b_to_d(rs1)]])) <<endl;
    ...
    if (c == "0100000011" && (bd((value[reg[b_to_d((rs1))]])) + imm) < 32  && (bd((value[reg[b_to_d((rs1))]])) + imm) > 0){ //lw  
       value[reg[b_to_d(rd)]] = data[mem[bd(value[reg[b_to_d(rs1)]]) + imm]];
      string result1 =  value[reg[b_to_d(rd)]];
      
    }

    else if(c == "0000010011") { //addi
        value[reg[b_to_d(rd)]] = sext(( bd((value[reg[b_to_d((rs1))]])) + imm),32);
        string result2 = value[reg[b_to_d(rd)]];
     
    }

     else if(c == "0110010011") {  //sltiu
...
void s_type(string fun){
  ...
  string c = (fun3 + opcode);
    ...
    if( c == "0100100011" && (bd((value[reg[b_to_d(rs1)]])) + imm) < 32 && (bd((value[reg[b_to_d(rs1)]])) + imm) > 0){
        data[mem[bd((value[reg[b_to_d(rs1)]])) + imm]] = value[reg[b_to_d(rs2)]];
...
void j_type (string fun){
  string imm = fun.substr(0,1)+fun.substr(12,8)+fun.substr(11,1)+fun.substr(1,10);
  int imm1 = bd(imm+"0");
...
int main(){
  ...
  for(int i=0;i<assm.size();i++){
    if(isPresent(arr1,1,assm.at(i).substr(25,7)))  {
      r_type(assm.at(i));
      print_value();
    }
    else if(isPresent(arr2,3,assm.at(i).substr(25,7))){
      i_type(assm.at(i));
       print_value();
    }
    else if(isPresent(arr4,1,assm.at(i).substr(25,7))){
      b_type(assm.at(i));
       print_value();
    }
    else if(isPresent(arr5,2,assm.at(i).substr(25,7))){
      u_type(assm.at(i));
       print_value();
    }
    else if(isPresent(arr6,1,assm.at(i).substr(25,7))){
      j_type(assm.at(i));
      print_value();
    }
  }
